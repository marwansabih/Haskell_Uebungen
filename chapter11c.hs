...
nextmoves :: Int-> Player -> Player -> Player -> [Grid] -> [Decisiontree]
nextmoves _ _ _ _ [] = []
nextmoves depth sp alpha beta (m:ms) = case sp of
                                                                  X -> if beta < p
                                                                           then [tree]
                                                                           else tree : ( nextmoves depth sp (max p alpha) beta ms)
                                                                           where
                                                                             tree@(Node(_,p) _) = ratetree' (depth -1) (next sp) alpha beta m
                                                                  O -> if alpha > p
                                                                        then [tree]
                                                                        else tree : (nextmoves depth sp alpha (min p beta) ms )
                                                                        where
                                                                           tree@(Node(_,p) _) = ratetree' (depth-1) (next sp) alpha  beta m

ratetree :: Player  -> Grid -> Decisiontree
ratetree sp g = ratetree' 7 sp SO GX g

ratetree' :: Int -> Player -> Player -> Player -> Grid -> Decisiontree
ratetree' depth sp alpha beta g | wins' O g = Node(g,O) []
                                                | wins' X g = Node(g,X) []
                                                | full g = Node(g,B) []
                                                | depth == 0 = Node(g,B) []
                                               ...
                                               where
                                                 ms = moves g sp
                                                 ratedtrees = nextmoves depth sp alpha beta ms
...
bestmove g  p = head [g' | Node (g', p') ts' <- ts, p' == best]
                           where
                               Node (_,best) ts = ratetree p g
